...
def process_single_mask(mask_path, valid_subtiles=None, max_per_tile=None, rng=None):
    """
            ...
            for r_idx in range(GRID_N):
                for c_idx in range(GRID_N):
                    ...
                    if len(rows) > 0:
                        ...
                        subtile_records.append(df_chunk)

    except Exception as e:
        print(f"[ERROR] Failed processing {mask_path}: {e}")
        return []

    if not subtile_records:
        return []

    tile_df = pd.concat(subtile_records, ignore_index=True)

    if max_per_tile and len(tile_df) > max_per_tile:
        rng = rng or np.random.default_rng()
        sample_idx = rng.choice(len(tile_df), size=max_per_tile, replace=False)
        tile_df = tile_df.take(sample_idx)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate global tree pixel index from masks."
    )
    parser.add_argument(
        "--mask_dir", required=True, help="Directory containing 100km .tif masks"
    )
    parser.add_argument(
        "--output_dir", required=True, help="Output directory for Parquet shards"
    )
    parser.add_argument(
        "--completed_tiles_file",
        required=False,
        help="Path to text file containing list of completed 20km tile IDs to filter by",
    )
    parser.add_argument(
        "--shard_rows", type=int, default=1_000_000, help="Approx rows per output file"
    )
    parser.add_argument(
        "--max_per_tile",
        type=int,
        default=None,
        help="Maximum trees to retain per 20km tile (random sample).",
    )
    ...
    if args.completed_tiles_file:
        ...
        if p.exists():
            ...
            print(
                f"Filtering index to {len(valid_subtiles)} completed tiles from {p.name}"
            )
        else:
            print(
                f"[WARN] Completed tiles file not found: {p}. Proceeding with ALL tiles."
            )
    ...
    for f in tqdm(mask_files):
        chunks = process_single_mask(f, valid_subtiles, args.max_per_tile, rng)
        if chunks:
            ...
            if current_len >= args.shard_rows:
                ...
                # Add random sort key
                full_df["rand_sort"] = np.random.random(size=len(full_df)).astype(
                    np.float32
                )
