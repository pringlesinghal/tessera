...
def main():
    parser = argparse.ArgumentParser(description="Globally sort/shuffle the tree index.")
    parser.add_argument("--input_dir", required=True, help="Directory containing original part-*.parquet files")
    parser.add_argument("--output_dir", required=True, help="Directory to save the new global shuffled index")
    parser.add_argument("--temp_dir", default="/scratch/users/psinghal/dask_temp", help="Scratch space for Dask shuffling")
    parser.add_argument("--memory_limit", default="32GB", help="Memory limit for Dask workers")
    ...
    if output_dir.exists():
        print(f"[WARN] Output directory {output_dir} exists. Please remove or specify a new one.")
        return
