#!/usr/bin/env python
# -*- coding: utf-8 -*-
# copied from train_multi_gpu_FSDP_QAT
...
)  # , quantize_tensor_symmetric, dequantize_tensor_symmetric # These two are not directly used in train script
from models.ssl_model import (
    MultimodalBTModel,
    BarlowTwinsLoss,
...
# Import TreeDataset for simplified data loading
import sys
...
# ChunkDataset class removed - now using TreeDataset instead
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="SSL Training (FSDP + chunk-based loading)"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/ssl_config.py",
        help="Path to config file (e.g. configs/ssl_config.py)",
    )
    return parser.parse_args()


def adjust_single_group_lr(
    iter_count,
    total_iters,
    learning_rate,
    warmup_ratio=0.1,
    ...
    if is_resume:
        ...
        if iter_count < warmup_iters:
            return (
                learning_rate * (iter_count / warmup_iters)
                if warmup_iters > 0
                else learning_rate
            )
        else:
            ...
            decay_progress = (iter_count - cosine_start_iter) / (
                total_iters - cosine_start_iter
            )
            decay_progress = max(0.0, min(1.0, decay_progress))
            cosine_decay = 0.5 * (1 + math.cos(math.pi * decay_progress))
            return learning_rate * cosine_decay
...
def save_fsdp_model(
    model, optimizer, config, filename, step, epoch, global_rank, is_final=False
):
    ...
    with FSDP.state_dict_type(
        ...
        full_model_state_config,
        optim_state_config,
    ):
        model_state = model.state_dict()
        if is_final:
            optim_state = FSDP.optim_state_dict(
                model, optimizer, optim_state_dict_config=optim_state_config
            )
    ...
    if global_rank == 0:
        ...
        logging.info(
            f"Model saved to {filename} (took {time.time() - start_time:.2f}s)"
        )
...
def load_checkpoint_for_resume(
    checkpoint_path, model, optimizer, scaler, device, global_rank
):
    """加载checkpoint用于恢复训练"""
    ...
    if "optimizer_state_dict" in checkpoint:
        ...
        optim_state = FSDP.optim_state_dict_to_load(model, optimizer, optim_state_dict)
...
def main():
    ...
    nnodes = (
        world_size // nproc_per_node if nproc_per_node > 0 else world_size
    )  # handle nproc_per_node = 0 if no CUDA
    ...
    if global_rank == 0:
        logging.info(f"Apply AMP (Automatic Mixed Precision) = {apply_amp}")

    if global_rank == 0:
        logging.info(
            f"Running on {world_size} GPU(s) across {nnodes} node(s). GlobalRank={global_rank}, LocalRank={local_rank}, device={device}"
        )
    ...
    if is_resume and global_rank == 0:
        logging.info(
            f"Resume training from checkpoint: {config['resume_from_checkpoint']}"
        )

    run_name = (
        f"FSDP_BT_QAT_{timestamp}"
        if config.get("apply_qat_representation")
        else f"FSDP_BT_{timestamp}"
    )
    if is_resume:
    ...
    if global_rank == 0:
        ...
        for f_path in relevant_files:
            if os.path.exists(f_path):
                ...
                logging.warning(f"W&B: Source file {f_path} not found for artifact.")
    ...
    if global_rank == 0:
        logging.info(
            f"Initial total steps estimate per rank (will be updated after dataset creation) = {total_steps_approx}"
        )

    qat_start_step_cfg = config.get("qat_representation_start_step", float("inf"))
    if (
        isinstance(qat_start_step_cfg, float) and 0 < qat_start_step_cfg < 1
    ):  # If it's a ratio
        config["qat_representation_start_step"] = int(
            total_steps_approx * qat_start_step_cfg
        )
        if global_rank == 0:
            logging.info(
                f"QAT for representation will start at step: {config['qat_representation_start_step']}"
            )
    elif not isinstance(
        qat_start_step_cfg, int
    ):  # If not ratio and not int, set to inactive
    ...
    s2_enc = TransformerEncoder(
        ...
        num_encoder_layers=config["s2_num_layers"],
    ...
    s1_enc = TransformerEncoder(
        ...
        num_encoder_layers=config["s1_num_layers"],
    ...
    if local_rank == 0:
        ...
        logging.info(
            f"Node {node_id}, LocalRank {local_rank} - Before FSDP - Total params: {sum(p.numel() for p in model.parameters()):,}"
        )
    ...
    if global_rank == 0:
        logging.info(
            f"FSDP Configuration: {json.dumps({k: str(v) for k, v in fsdp_config_dict.items()}, indent=2)}"
        )
    ...
    if global_rank == 0:
        logging.info(
            f"Model structure after FSDP wrapping (first 5000 chars):\n{str(model)[:5000]}..."
        )
        fsdp_params = sum(p.numel() for p in model.parameters())
        logging.info(
            f"After FSDP wrapping - Parameters on rank {global_rank}: {fsdp_params:,}"
        )
        logging.info(f"GPU memory usage after FSDP: {get_gpu_memory_usage():.2f} MB")

    if config.get("use_torch_compile", False):
        if global_rank == 0:
            logging.info("Attempting to torch.compile the FSDP model...")
        try:
            ...
            if global_rank == 0:
                logging.info("torch.compile successfully applied.")
        except Exception as e:
            if global_rank == 0:
                logging.error(f"torch.compile failed: {e}. Proceeding without compile.")
    ...
    if global_rank == 0:
        ...
        if not weight_params and all_params:
            logging.warning(
                "No weight parameters found based on shape! Using all params in one group."
            )
    ...
    if (
        not weight_params and not bias_params and all_params
    ):  # Only if both are empty but all_params is not
        optimizer_param_groups = [
            {"params": all_params, "weight_decay": config.get("weight_decay", 1e-6)}
        ]
    else:
        if weight_params:
        ...
        if (
            not optimizer_param_groups and all_params
        ):  # Fallback if logic somehow misses params
            optimizer_param_groups = [
                {"params": all_params, "weight_decay": config.get("weight_decay", 1e-6)}
            ]
    ...
    if is_resume:
        try:
            ...
            if global_rank == 0:
                logging.info(
                    f"Successfully resumed training from step {g_step}, epoch {start_epoch}"
                )
                logging.info(
                    f"Estimated examples processed so far: {examples_processed_total}"
                )
        except Exception as e:
            if global_rank == 0:
    ...
    if global_rank == 0 and wandb_run:
        wandb.watch(
            model, log="gradients", log_freq=config.get("wandb_watch_freq", 400)
        )
    ...
    if "field_id_path" in config and os.path.exists(config["field_id_path"]):
        ...
        if global_rank == 0:
            logging.info(
                f"Field IDs loaded, shape: {field_ids.shape if field_ids is not None else 'None'}"
            )
    ...
    for epoch in range(start_epoch, config["epochs"]):
        # Initialize TreeDataset for this epoch
        if global_rank == 0:
            logging.info(f"Epoch {epoch} starting, initializing TreeDataset...")
        ...
        if global_rank == 0:
            logging.info(f"Dataset initialized with {len(dataset_train)} samples")

        # Update total_steps on first epoch based on actual dataset size
        if epoch == start_epoch:
            total_steps = config["epochs"] * len(dataset_train) // world_size
            total_steps_approx = total_steps
            if global_rank == 0:
                logging.info(f"Updated total steps based on dataset: {total_steps}")

        # Create distributed sampler
        # Dataset is already randomized, so we use serial loading (shuffle=False)
        train_sampler = DistributedSampler(
            dataset_train,
            num_replicas=world_size,
            rank=global_rank,
            shuffle=False,  # Serial loading since dataset is already randomized
            drop_last=True,
        )
        ...
        # Note: Normalization is handled in the dataset (TreeDataset) using parameters
        # defined in tessera_ml/dataset.py. Config "normalize" controls this.
        # Always use normalize=True in config for consistent behavior.

        # Main training loop (no gradient accumulation)
        for batch_data in train_loader:
            # Data is already normalized by the dataset
            ...
            if not flops_computed_once and FlopCountAnalysis is not None:
                try:
                    ...
                    if global_rank == 0 and single_forward_flops > 0:
                        logging.info(
                            f"Computed FLOPs for single forward pass: {single_forward_flops:,}"
                        )
                except Exception as e:
                    if global_rank == 0:
            ...
            try:
                ...
                if hasattr(model, "module"):
                    ...
                    logging.warning(
                        "Could not set qat_active_for_current_step on model or model.module."
                    )

            if (
                qat_is_active_this_step
                and global_rank == 0
                and g_step == config.get("qat_representation_start_step")
            ):
                logging.info(
            ...
            if len(rolling_loss_list) > 40:
                rolling_loss_list.pop(0)
            avg_rolling_loss = (
                sum(rolling_loss_list) / len(rolling_loss_list)
                if rolling_loss_list
                else 0.0
            )
            ...
            if (
                ...
                and g_step > 0
                and g_step % config["val_interval_steps"] == 0
            ):
                ...
                checkpoint_path_val = os.path.join(
                    checkpoint_dir_val, f"checkpoint_{timestamp}_step{g_step}.pt"
                )

                saved_model_state_dict_rank0 = save_fsdp_model(
                    model,
                    optimizer,
                    config,
                    checkpoint_path_val,
                ...
                if global_rank == 0:
                    logging.info(
                        f"Starting validation at global_step {g_step} using checkpoint {checkpoint_path_val}..."
                    )
                    val_config_keys = [
                    ...
                    if not all(config.get(k) for k in val_config_keys):
                        logging.warning(
                            "Validation data paths missing in config. Skipping validation."
                        )
                    else:
                        try:
                            ...
                            val_dataset = AustrianCropValidation(  # Ensure this class is correctly defined and imported
                                ...
                                s1_asc_bands_file_path=config[
                                    "val_s1_asc_bands_file_path"
                                ],
                                s1_asc_doy_file_path=config["val_s1_asc_doy_file_path"],
                                s1_desc_bands_file_path=config[
                                    "val_s1_desc_bands_file_path"
                                ],
                                s1_desc_doy_file_path=config[
                                    "val_s1_desc_doy_file_path"
                                ],
                                ...
                                sample_size_s2=config["sample_size_s2"],
                                sample_size_s1=config["sample_size_s1"],
                                min_valid_timesteps=config.get(
                            ...
                            val_loader_eval = DataLoader(
                                val_dataset,
                                batch_size=config.get("val_batch_size", 512),
                                shuffle=False,
                            ...
                            if saved_model_state_dict_rank0:
                                ...
                                for (
                                    k_val,
                                    v_val,
                                ) in saved_model_state_dict_rank0.items():
                                    nk_val = k_val
                                    if k_val.startswith("_fsdp_wrapped_module."):
                                        nk_val = k_val[len("_fsdp_wrapped_module.") :]
                                    elif k_val.startswith("_orig_mod."):
                                        nk_val = k_val[len("_orig_mod.") :]
                                    clean_state_dict_val[nk_val] = v_val
                                eval_model_val.load_state_dict(
                                    clean_state_dict_val, strict=True
                                )
                            else:
                                eval_model_val = load_model_from_checkpoint(
                                    checkpoint_path_val, config, device
                                )
                            ...
                            val_acc, val_f1, val_cm, val_cr = linear_probe_evaluate(
                                ...
                                val_test_split_ratio=config.get(
                                ...
                                device=device,
                                apply_amp=apply_amp,
                                apply_qat_representation_for_eval=config.get(
                                    "apply_qat_representation", False
                                ),
                                qat_representation_bits_for_eval=config.get(
                                    "qat_representation_bits", 8
                                ),
                            )
                            logging.info(
                                f"Validation at Step {g_step}: Accuracy={val_acc:.4f}, F1={val_f1:.4f}"
                            )
                            if wandb_run:
                                wandb.log(
                                    {
                                        ...
                                        "val_classification_report_html": wandb.Html(
                                            f"<pre>{val_cr}</pre>"
                                        ),
                                    },
                                    step=g_step,
                                ...
                                try:  # Plot confusion matrix
                                    ...
                                    logging.warning(
                                        f"Could not plot confusion matrix for W&B: {e_cm_plot}"
                                    )

                            if val_acc > best_val_acc:
                                best_val_acc = val_acc
                                logging.info(
                                    f"New best validation accuracy: {best_val_acc:.4f}. Saving best model."
                                )
                                best_ckpt_path_val = os.path.join(
                                    checkpoint_dir_val,
                                    f"best_model_fsdp_{timestamp}.pt",
                                )
                                torch.save(
                                    {
                                        "epoch": epoch,
                            ...
                            logging.error(
                                "Could not import AustrianCropValidation. Ensure datasets/ssl_dataset.py is correct and in PYTHONPATH."
                            )
                        except Exception as e_val:
                            logging.error(
                                f"Validation error at step {g_step}: {e_val}",
                                exc_info=True,
                            )
                        finally:
                            torch.cuda.empty_cache()
        ...
        if global_rank == 0:
            logging.info(
                f"Epoch {epoch} finished. Duration: {epoch_duration:.2f}s. Total examples in epoch (global): {epoch_examples_processed}"
            )
            logging.info(
                f"GPU memory at end of epoch {epoch}: {get_gpu_memory_usage():.2f} MB"
            )
            if wandb_run:
                wandb.log(
                    {
                        "epoch_duration_sec": epoch_duration,
                        "epoch_examples_global": epoch_examples_processed,
                    },
                    step=g_step,
                )
    ...
    if global_rank == 0:
        ...
        final_checkpoint_path = os.path.join(
            "checkpoints", "ssl", f"checkpoint_{timestamp}_final.pt"
        )
        save_fsdp_model(
            model,
            optimizer,
        ...
        if wandb_run:
            ...
            best_model_path_check = os.path.join(
                "checkpoints", "ssl", f"best_model_fsdp_{timestamp}.pt"
            )
            if os.path.exists(best_model_path_check):
                best_model_artifact = wandb.Artifact(
                    f"best_model_{timestamp}",
                    type="model",
                    metadata={"best_val_acc": best_val_acc, "step": g_step},
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if dist.get_rank() == 0:  # Print only on one rank
            print(
                "Warning: scikit-learn ConfusionMatrixDisplay not available. Confusion matrix plots to W&B might fail."
            )
            print(
                "Consider: pip install scikit-learn>=0.22 (or check your sklearn version)"
            )
